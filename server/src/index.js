...
const COLONNES_EVENT =
  'id, theme_id, name, description, event_date, location, guests, budget::float AS budget, image_url, image_focus, position, created_at, hidden_fields, labels';
...
app.patch('/api/events/:id', exigerAdmin, a(async (req, res) => {
  const evenement = await patch(
    ...
    ['name', 'description', 'event_date', 'location', 'guests', 'budget', 'image_url', 'image_focus', 'hidden_fields', 'labels'],
    COLONNES_EVENT
  );
